...
import { unpublishArticle } from "@/lib/articles-api";
import { getSessionToken } from "@/lib/session";

/** PATCH /api/admin/articles/[id]/unpublish — proxies to PATCH /api/v1/articles/{id}/unpublish. */
export async function PATCH(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const article = await unpublishArticle(token, id);
