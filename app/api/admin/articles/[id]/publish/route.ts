...
import { publishArticle } from "@/lib/articles-api";
import { getSessionToken } from "@/lib/session";

/** PATCH /api/admin/articles/[id]/publish — proxies to PATCH /api/v1/articles/{id}/publish. */
export async function PATCH(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const article = await publishArticle(token, id);
